...
class GradRequirementsDB(object):
    ...
    # 졸업요건 항목 추가하기
    # args : (major, sub_major, field, field_name, value, description)
    ...
    # 전공, 전공2 주면 해당하는 졸업요건 리스트 전체 가져오기
    # args : (major, sub_major)
    # 6개 정보를 담은 튜플의 리스트로 반환됨
    def list_requirement(self, args: tuple):
        self.cur.execute('SELECT * FROM grad_requirements WHERE major=? AND sub_major=?', args)
    ...
    # 졸업요건 항목 수정하기
    # args : (major, sub_major, field, field_name, value, description)
    # 해당하는 (major, sub_major, field)의 값을 새 값들로 모두 바꿈
    def edit_requirement(self, args: tuple):
        (major, sub_major, field, field_name, value, description) = args
        self.cur.execute(f'UPDATE grad_requirements SET field_name={field_name}, value={value}, description={description} WHERE major=? AND sub_major=? AND field=?', (major, sub_major, field))
        self.conn.commit()
